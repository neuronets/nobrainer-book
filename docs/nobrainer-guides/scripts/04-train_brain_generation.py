...
#
# In this notebook, we train a Progressive GAN on real brain MRI data using
# PyTorch Lightning. The volumes are downsampled to a small resolution for
# fast training on CPU.
#
# We will:
# 1. Download real brain MRI data
# 2. Normalize and downsample the volumes
...
# %%
<<<<<<< HEAD
# uv pip install nilearn nobrainer
...
# downsample to 4^3 for fast GAN training.
...
#
# We use a single resolution level for this demo. For multi-resolution
# training, pass e.g. `resolution_schedule=[4, 8, 16, 32, 64]`.
...
# ## Next steps
#
# - Train with multi-resolution schedule for higher quality output
# - Use more training steps and larger latent size
# - Save checkpoint: `trainer.save_checkpoint("progressivegan.ckpt")`
# - Generate via CLI: `nobrainer generate --model progressivegan.ckpt ...`
